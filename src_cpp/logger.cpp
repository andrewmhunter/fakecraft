...
void vprintLog(TraceLogLevel level, const char* where, const char* format, va_list args) {
    static const std::map<TraceLogLevel, const char*> levelText {
        {LOG_ALL, "ALL"},
    ...
    fprintf(stderr, "%s\x1b[0m: %s: ", levelText.at(level), where);
...
void raylibLogCallback(int level, const char* format, va_list args) {
    vprintLog(static_cast<TraceLogLevel>(level), "raylib", format, args);
}
